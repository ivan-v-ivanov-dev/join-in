package com.social.notification.service.constants;

public class ServiceConstants {

    public static final String COLLECTION_TEMPLATE = "c_%s";

    public static final String NEW_POST_NOTIFICATION_TEMPLATE = "%s shared a new post";

    public static final String NEW_COMMENT_NOTIFICATION_TEMPLATE = "%s wrote a new comment";

    public static final String LIKES_POST_NOTIFICATION_TEMPLATE = "%s likes a post";

    public static final String DISLIKES_POST_NOTIFICATION_TEMPLATE = "%s disliked a post";

    public static final String STARS_POST_NOTIFICATION_TEMPLATE = "%s stared a post";

    public static final String LIKE_COMMENT_NOTIFICATION_TEMPLATE = "%s likes your comment";

    public static final String DISLIKE_COMMENT_NOTIFICATION_TEMPLATE = "%s dislikes your comment";

    private ServiceConstants() {
    }
}
